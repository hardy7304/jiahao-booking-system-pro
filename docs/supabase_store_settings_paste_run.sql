-- =============================================================================
-- 首頁 Landing CMS：store_settings（與 Cursor 專案 migration 對齊）
-- 使用方式：Supabase Dashboard → SQL Editor → 貼上 → Run
--
-- 前置：需已有 public.stores，且種子用的 store_id 在 stores 裡存在。
-- 預設種子店 id：8e8388bf-860e-44f7-8e14-35b76c64fb52（安平店，與程式 FALLBACK 一致）
-- 若 INSERT 報 FK 錯誤：先確保該 uuid 在 stores，或刪掉最後一段 INSERT。
-- =============================================================================

-- 1) 建表（若已存在則略過；舊表請靠下方 ALTER 補欄）
CREATE TABLE IF NOT EXISTS public.store_settings (
  ...
  studios_shell JSONB NOT NULL DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- 2) 舊表若早於 studios_shell / is_roushou_visible 建立，補欄（不會重複）
...
-- 若 is_roushou_visible 曾為 nullable，可選：填滿再設 NOT NULL（有錯就略過）
-- UPDATE public.store_settings SET is_roushou_visible = true WHERE is_roushou_visible IS NULL;

CREATE INDEX IF NOT EXISTS idx_store_settings_store_id ON public.store_settings(store_id);

-- 3) RLS（與專案 migration 一致：公開讀寫；正式上線請再依多租戶收斂）
